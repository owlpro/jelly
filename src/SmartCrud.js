...
class SmartCrud extends Component {
        ...
        return (
            <Box p={3} className="smartcrud_create_section_wrapper" style={{ backgroundColor: "white" }}>
                <Row>
                    ...
                    <Col xs={12}>
                        {this.props.hasOne ? this.props.hasOne.map((relation, rel_index) => <HasOne key={rel_index} index={rel_index} {...relation} ref={el => this.relations.hasOne[rel_index] = el} />) : null}
                    </Col>
                    <Col xs={12}>
                        {this.props.hasMany ? this.props.hasMany.map((relation, rel_index) => <HasMany key={rel_index} index={rel_index} {...relation} ref={el => this.relations.hasMany[rel_index] = el} />) : null}
                    </Col>
                    <Col xs={12}>
                        <Button onClick={this.onSubmit} variant={'contained'} className='ml-2 shadow-0' color={'primary'}>ارسال</Button>
                        <Button onClick={this.onClear} variant={'contained'} className="shadow-0">پاک کردن</Button>
                    </Col>
                </Row>
            </Box>
        )
        ...
        return (
            <Box p={1}>
                <Row>
                    ...
                    <Col xs={12}>
                        {this.props.hasOne ? this.props.hasOne.map((relation, rel_index) => {
                            ...
                            return <HasOne {...this.props} row={this.state.edit.data} key={rel_index} index={rel_index} {...relationClone} ref={el => this.relations.hasOne[rel_index] = el} />
                        }) : null}
                    </Col>
                    <Col xs={12}>
                        {this.props.hasMany ? this.props.hasMany.map((relation, rel_index) => {
                            let relationClone = { ...relation };
                            let selector = relation.relation.split(':')[0];
                            let value = params.data ? params.data[selector] : null;
                            relationClone['value'] = value;

                            return <HasMany {...this.props} row={this.state.edit.data} key={rel_index} index={rel_index} {...relationClone} ref={el => this.relations.hasMany[rel_index] = el} />
                        }) : null}
                    </Col>
                </Row>
            </Box>
    ...
    renderEditModal = () => {
        return (
            <Modal
                ...
                onHide={this.setEditModalTo(false)}>
                <Modal.Header>
                    <Modal.Title>
                        <span>ویرایش</span>
                        &nbsp;
                ...
                <Modal.Body className="settlementWithmerchantModal">
                    <Box p={3}>
                        {this.renderEditContent(this.state.edit)}
                    </Box>
                </Modal.Body>

                <Modal.Footer className="justify-content-center">

                    <Button variant="contained" className="shadow-0" color="primary" onClick={this.onSubmitEdit}>ذخیره</Button>
                    <Button variant="contained" className="mr-2 shadow-0" color="default" onClick={this.setEditModalTo(false)}>بستن</Button>
    ...
    exportData = () => {
        ...
        let values = this.state.inputs.map(item => {
            ...
            if (this.state.edit.data) { // if is edit mode and is editable
                if (item.editable) {
                    return {
    ...
    onDelete = row => {
        Dialog({
            title: "حذف",
            description: (<div>
                <p>از حذف <b>{row.title || row.subtitle}</b> اطمینان دارید ؟</p>
            </div>),
            yes: "تایید و حذف",
            onAccept: () => {
...
export default connect(mapStateToProps, mapDispatchToProps)(SmartCrud);