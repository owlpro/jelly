...
class Media extends Component {
    ...
    searchMedia = () => {
        ...
        if (value) {
            this.setState(state => {
                let ns = { ...state };
                ns.searchedMedia = this.props.media.filter(item => item.users[0].pivot.title.toLowerCase().indexOf(value.toLowerCase()) > -1 || ("." + item.format).indexOf(value) > -1)
                ns.inSearch = true;
                return ns;
            })
                                        ...
                                        >
                                            <Col xs={12} className="sm_media_type_categories_inner">
                                                <Row className="sm_media_type_categories">
                                                    {formats.length ? formats.map((format, key) => (
                                                        <Col xs={6} className="sm_media_type_category_wrapper" key={key}>
                                                            <Button onClick={this.filterByCategory(format)} variant="contained" color="default" className="sm_media_category_item shadow-0">
                                    ...
                                    >
                                        ...
                                        <div className="sm_media_contents">
                                            <Col className="sm_media_contents_inner m-0">
                                                <Row className="sm_media_item_wrapper">
                                                    {mediaToShow.map(item => (
                                                        this.renderMediaItem(item)
                                                    ))}
                                                </Row>

                                                {mediaToShow.length < 1 ? this.state.inSearch && !this.props.loading ? (
    ...
    renderUploadStatus = item => {
        ...
        switch (item.event.status) {
            ...
            case "success": return <span className="upload_success_icon"><i className="feather icon-check"></i></span>;
            case "failed": return <span className="upload_failed_icon"><i className="feather icon-x"></i></span>;
        }
                        ...
                        {this.state.uploadingItems.map(item => (
                            <div className={(item.event.finished ? "sm_uploading-item_inner_finished" : "") + " sm_uploading-item_inner"} key={item.key}>
                                <div className="sm_uploading-item">
                                    <div className="sm_uploading_status">
                                        {this.renderUploadStatus(item)}
                                    </div>
                                    <div className="sm_uploading_name">
                                        {item.file.name}
                                    </div>
                                </div>
                            </div>
                ...
                <div className={(this.state.details.show ? "" : "sm_dt_wrapper_hide ") + "sm_dt_wrapper"}>
                    ...
                    <div className="dt_sm_file_contents">
                        ...
                        <div className="sm_dt_info_section">
                            <label htmlFor="" className="sm_dt_info_label">نام فایل</label>
                            <input type="text" readOnly className="sm_dt_info" value={media.users[0].pivot.title} />
                        </div>
                        <div className="sm_dt_info_section">
                            <label htmlFor="" className="sm_dt_info_label">آدرس فایل</label>
                            <input type="text" readOnly className="sm_dt_info" value={path} onClick={this.copy} />
                        </div>
                        {this.#isImage(media) ? (
                            <div className="sm_dt_info_section">
                                <label htmlFor="" className="sm_dt_info_label">ابعاد فایل</label>
                        ...
                        <div className="sm_dt_info_section">
                            <label htmlFor="" className="sm_dt_info_label">توضیحات</label>
